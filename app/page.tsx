...
// Import the new optimized follow button
    ...
    try {
      ...
      const memeIdSets = await Promise.all(
        selectedHashtags.map((hashtag) =>
          HashtagService.getMemesByHashtag(hashtag, 100)
        )
      );
      ...
      const filteredMemes = allMemes.filter((meme) =>
        commonMemeIds.includes(meme.id)
      );
        ...
        {feedType === "following" ? (
          ...
          <>
            ...
            {memes.length === 0 ? (
              ...
              >
                {memes
                  .filter((meme) => meme && meme.id && meme.publicId)
                  .map((meme, index) => (
                    <motion.div
                      key={meme.id}
                      id={`meme-${meme.id}`}
