...
export default function LikedMemesPage() {
  ...
  const [likedInteractions, setLikedInteractions] = useState<UserInteraction[]>([]);
  ...
  useEffect(() => {
    ...
    const loadLikedMemes = async () => {
      try {
        // Get user's liked interactions
        const interactions = await UserInteractionService.getUserLikedMemes(userId, 100);
        setLikedInteractions(interactions);

        // Get the actual meme data for each liked meme
  ...
  const canViewLikedMemes = user && (user.uid === userId);
  ...
  if (!canViewLikedMemes) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-text-primary mb-2">Access Denied</h1>
          <p className="text-text-secondary">You can only view your own liked memes.</p>
          <button
            onClick={() => router.back()}
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="mt-1 sm:mt-2 text-sm sm:text-base text-text-secondary">
                  Memes you've liked ({likedMemes.length})
                </p>
              </div>
            </div>
          </div>
        ...
        {memesLoading ? (
          ...
          >
            ...
            <p className="text-sm sm:text-base text-text-secondary mb-4 sm:mb-6">
              Start exploring and liking memes to see them here!
            </p>
            <motion.button
